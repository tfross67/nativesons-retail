...
FIXES = [
    ...
    # Turqouise → Turquoise (spelling)
    ("Turqouise", "Turquoise"),
    # co<on → cotton (OCR: < rendered as t)
    ("co<on", "cotton"),
    # Origin: consolidate Hybrid variants to "Garden hybrid"
    ("California, Hybrid", "Garden hybrid"),
    # Cultivar name OCR: # → t (Ascot Pe#te, mar#nii)
    ("Ascot Pe#te", "Ascot Petite"),
    ("mar#nii", "martinii"),
    # Cultivar name OCR: @ → tt (Flu@er)
    ("Flu@er", "Flutter"),
    # Cultivar name OCR: I → t (Bronze LiIle Leaf)
    ("LiIle Leaf", "Little Leaf"),
    ("LiIle", "Little"),
    # Common name OCR (capital letters mid-word that should be lowercase t/a/f)
    ("BuDerfly", "Butterfly"),
    ("AdriaIc", "Adriatic"),
    ("PeAte", "Petite"),
    ("mariFma", "maritima"),
    ("LiDle", "Little"),
    ("myrFfolia", "myrtifolia"),
    ("HunEngton", "Huntington"),
    ("byzanFna", "byzantina"),
    ("fruFcans", "fruticans"),
    # Botanical OCR: la#folia → latifolia (Epilobium)
    ("la#folia", "latifolia"),
    # Botanical OCR: EvereE's → Everett's
    ("EvereE's", "Everett's"),
]
...
def fix_record(p):
    ...
    # Normalize standalone "Hybrid" origin to "Garden hybrid"
    if new_orig == "Hybrid":
        new_orig = "Garden hybrid"

    # Prostanthera ovalifolia 'Variegata' common name has a row-mash:
    # the width value "4-6'" got appended to the common name in the PDF.
    if new_bot == "Prostanthera ovalifolia 'Variegata'":
        new_com = "Variegated Australian mint bush"

    # Epilobium canum ssp. latifolia 'Everett's Choice' has "California"
    # mashed into the botanical name (it's the origin). Use curly quotes
    # since the PDF emits ‘ ’ rather than ' '.
    if new_bot == "Epilobium canum ssp. latifolia ‘EvereE’s Choice’California":
        new_bot = "Epilobium canum ssp. latifolia ‘Everett’s Choice’"
        if not new_orig:
            new_orig = "California"
    elif new_bot == "Epilobium canum ssp. latifolia 'EvereE's Choice'California":
        new_bot = "Epilobium canum ssp. latifolia 'Everett's Choice'"
        if not new_orig:
            new_orig = "California"
